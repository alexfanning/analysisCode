...
saccadeThresh = 200;                            %eye velocity mean squared error threshold for what is considered a saccade (linear desaccading; typically between 100 and 2000)
...
if condition ~= 4
    ...
    chP = data(1,dataSubset(2)).data*-1;                        %turntable position (the sign for the turntable in d019 or d241a isn't flipped properly during acquistion)
    chV = data(1,dataSubset(1)).data*-1;
end
...
if protocol == 0
    ...
    endVisual = SampKeyTimes(strfind(SampKeys, 'XLSxss')+4);
