...
if ~isUsingSaccades
    % Saccades
    saccadeCenters = detectSaccades(magnetTime, magnetPos, magnetVel, 1000, 5, true);
    saccades = getStartStop(saccadeCenters);
    
    if isRemovingSegment
        removeIndices = find(saccades(:, 1) > removeTime(1) * 1000 & saccades(:, 2) < removeTime(2) * 1000);
    ...
    [saccades, artifacts] = removeArtifacts(saccades, magnetPos);
end
