...
goodCycMin = 5;                 %minimum number of cycles per block needed to add block to cycle-average
...
if (protocol == 0 || protocol == 1) && (condition == 0 || condition == 1 || condition == 2) && frequency == 1
...
elseif (protocol == 0 || protocol == 1) && (condition == 0 || condition == 1 || condition == 2) && frequency == 0.5
    numTimeVisual = round((endVisual(1)/1000)-(startVisual(1)/1000));           %number of seconds for each testing condition for reshaping purposes (visual period)
    numTimeGap = round((endGap(2)/1000)-(startGap(2)/1000));                    %gap period
    numTimeDark = round((endDark(1)/1000)-(startDark(1)/1000))-1;                 %vestibular-dark period
...
if numBlocksDark > 0
    ...
    for i = 1:numBlocksDark
        ...
        [desaccadeBlock{i},desaccadeBlockTrunc{i},desaccadeBlockMat{i}] = segmenting(desaccade2,startDark,startGap,endDark,nthTime,fr,frequency,numTimeDark,keep,visualRef);
        nthTime = nthTime + 1;
    end
end
...
if numBlocksVisual > 0 && protocol ~= 3 && condition ~= 3
    ...
    for i = 1:numBlocksVisual
        ...
        [desaccadeBlock{i,2},desaccadeBlockTrunc{i,2},desaccadeBlockMat{i,2}] = segmenting(desaccade2,startVisual,startGap,endVisual,nthTime,fr,frequency,numTimeVisual,keepL,visualRef);
        nthTime = nthTime + 1;
    end
elseif numBlocksVisual > 0 && protocol == 3
    ...
    for i = 1:numBlocksVisual
        ...
        [desaccadeBlock{i,2},desaccadeBlockTrunc{i,2},desaccadeBlockMat{i,2}] = segmenting(desaccade2,startVisual,startRec,endVisual,nthTime,fr,frequency,numTimeVisual,keepL,visualRef);
        nthTime = nthTime + 1;
    end
end
visualRef = 0;
...
if numBlocksGap > 0 && (protocol == 0 || protocol == 1 || protocol == 2)
    ...
    for i = 1:numBlocksGap
        ...
        [desaccadeBlock{i,3},desaccadeBlockTrunc{i,3},desaccadeBlockMat{i,3}] = segmenting(desaccade2,startGap,startGap,endGap,nthTime,fr,frequency,numTimeGap,keepSp,visualRef);
        nthTime = nthTime + 1;
    end
...
if protocol ~= 3 & expSubType ~= 1
    ...
    desaccadeDark = [desaccadeBlockMat{1:numBlocksDark,1}];
    desaccadeVisual = [desaccadeBlockMat{1:numBlocksVisual,2}];
    desaccadeGap = [desaccadeBlockMat{1:numBlocksGap,3}];
elseif expSubType == 1
    calDarkMatIn = [calciumRawBlockMat{1:numBlocksDark,1}]';
    ...
    eyePgapMatIn = [eyePrawBlockMat{1:numBlocksGap,3}];
    desaccadeDark = [desaccadeBlockMat{1:numBlocksDark,1}];
    desaccadeVisual = [desaccadeBlockMat{1:2:numBlocksVisual*2,2}];
    desaccadeVisual2 = [desaccadeBlockMat{2:2:numBlocksVisual*2,2}];
    desaccadeGap = [desaccadeBlockMat{1:numBlocksGap,3}];
...
%     desacSp = removeAroundCenters(eyeGapSacVector,shift,windowsize);
    desaccadeDark = desaccadeDark(:)';
    desaccadeVisual = logical(desaccadeVisual(:))';
    desaccadeGap = desaccadeGap(:)';
    calVisualMatIn = calVisualMatIn';
    calDarkMatIn = calDarkMatIn';
    calGapMatIn = calGapMatIn';
    calDesac = calVisualMatIn(:)';
    calDarkDesac = calDarkMatIn(:)';
    calSpDesac = calGapMatIn(:)';
    calDesac(desaccadeVisual) = NaN;
    calDarkDesac(desaccadeDark) = NaN;
    calSpDesac(desaccadeGap) = NaN;
    ...
    if condition == 4
        desaccadeVisual2 = logical(desaccadeVisual2(:))';
        calVisualMatIn2 = calVisualMatIn2';
        calDesac2 = calVisualMatIn2(:)';
        calDesac2(desaccadeVisual2) = NaN;
        calVisualMatOut2 = reshape(calDesac2,(1/frequency)*fr,numTimeVisual*frequency*numBlocksVisual);
    end
...
%     bar(keepIdxLength)
    ...
    if expSubType ~= 3 && expSubType ~= 5
        ...
        a = 1;
        for t = 1:numTimeVisual*frequency:size(rsDesacMat,2);
            rsBlockDesac{a} = rsDesacMat(:,t:t+numTimeVisual*frequency-1);
            a = a + 1;
        end
        ...
        if condition == 4
            ...
            a = 1;
            for t = 1:numTimeVisual*frequency:size(rsDesacMat2,2);
                rsBlockDesac2{a} = rsDesacMat2(:,t:t+numTimeVisual*frequency-1);
                a = a + 1;
            end
    ...
    if protocol ~= 3 && expSubType ~= 4 && expSubType ~= 3 && expSubType ~= 5
...
%%Gain calculations
if expSubType ~= 3 && expSubType ~= 5 && protocol ~= 3
